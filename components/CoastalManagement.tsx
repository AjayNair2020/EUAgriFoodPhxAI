...
const CoastalManagement: React.FC = () => {
  const [activeSubTab, setActiveSubTab] = useState<'monitoring' | 'data-collection'>('monitoring');
  ...
  const sensorStreamData = useMemo(() => Array.from({ length: 24 }, (_, i) => ({
    time: `${i}:00`,
    temp: 18 + Math.random() * 4,
    salinity: 35 + Math.random() * 0.5,
    tide: 2 + Math.sin(i / 3) * 1.5 + Math.random() * 0.2
  })), []);
  ...
  const liveSensors = [
    { name: 'Sea Temp', val: '19.4', unit: '°C', status: 'Optimal', icon: 'fa-temperature-full', color: 'text-rose-500' },
    { name: 'Salinity', val: '35.2', unit: 'psu', status: 'Stable', icon: 'fa-vial', color: 'text-blue-400' },
    { name: 'pH Level', val: '8.1', unit: 'pH', status: 'Normal', icon: 'fa-droplet', color: 'text-cyan-400' },
    { name: 'Tide Height', val: '2.4', unit: 'm', status: 'Rising', icon: 'fa-arrow-up-wide-short', color: 'text-emerald-500' }
  ];
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl border border-zinc-200 dark:border-zinc-800">
          <button 
            ...
            className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${activeSubTab === 'monitoring' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-blue-500'}`}
          >
            MONITORING
          </button>
          <button 
            onClick={() => setActiveSubTab('data-collection')}
            className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${activeSubTab === 'data-collection' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-blue-500'}`}
          >
            DATA COLLECTION
          </button>
        </div>
      ...
      {activeSubTab === 'monitoring' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn">
           <div className="lg:col-span-2 glass-panel p-6 rounded-[32px] shadow-xl h-[450px]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-black text-zinc-900 dark:text-white uppercase tracking-tighter">Live Sensor Data Collection</h3>
              <div className="flex space-x-4 text-[9px] font-black">
                <span className="text-rose-500">● SEA TEMP</span>
                <span className="text-emerald-500">● TIDE HEIGHT</span>
              </div>
            </div>
            <ResponsiveContainer width="100%" height="80%">
              <LineChart data={sensorStreamData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#3f3f46" opacity={0.1} />
                <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{fontSize: 10}} />
                <YAxis hide />
                <Tooltip contentStyle={{backgroundColor: '#091a0f', border: 'none', borderRadius: '16px'}} />
                <Line type="monotone" dataKey="temp" stroke="#f43f5e" strokeWidth={3} dot={false} />
                <Line type="monotone" dataKey="tide" stroke="#10b981" strokeWidth={3} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="glass-panel p-6 rounded-[32px] shadow-xl flex flex-col justify-between">
            <div>
              <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-6 uppercase tracking-tighter">Active Sensor Array</h3>
              <div className="grid grid-cols-1 gap-3">
                {liveSensors.map(sensor => (
                  <div key={sensor.name} className="p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-xl bg-white dark:bg-zinc-800 flex items-center justify-center shadow-sm ${sensor.color}`}>
                      <i className={`fas ${sensor.icon}`}></i>
                    </div>
                    <div className="flex-1">
                      <div className="text-[10px] font-bold text-zinc-500 uppercase leading-none mb-1">{sensor.name}</div>
                      <div className="flex items-baseline space-x-1">
                        <span className="text-xl font-black text-zinc-900 dark:text-white">{sensor.val}</span>
                        <span className="text-xs font-bold text-zinc-400">{sensor.unit}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">{sensor.status}</span>
                      <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse ml-auto mt-1"></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-6 p-4 bg-blue-500/5 rounded-2xl border border-blue-500/10">
               <div className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1 flex items-center">
                 <i className="fas fa-satellite mr-2"></i>
                 Satellite Uplink
               </div>
               <p className="text-[10px] text-zinc-500 leading-tight">Data synchronized with EU Copernicus Sentinel-3 mesh.</p>
            </div>
          </div>
        </div>
      )}
